...
for (s in seeds){
  print(paste0("Seed: ",s))
  set.seed(s)
  aa[[s]]<-stepArchetypes(phe, k=test_archetypes, nrep=nrep, method=robustArchetypes)
}


## check how different are the best archetype solutions across the different seed values

  best_archetypes=lapply(aa, function(x) bestModel(x))  
  ...
  pdf(paste0("4_archetypes.stability_by_threshold.pdf"))
