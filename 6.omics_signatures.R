...
my_files = list.files(path=filepath.olink, pattern = "*binder.csv", include.dirs = TRUE, full.names=TRUE)
...
my_files <- list.files(path=filepath.olink, pattern = "*NPX.csv", include.dirs = TRUE, full.names=TRUE)
...
names(olink) <- c("olink.cam","olink.cvd2","olink.cvd3","olink.dev","olink.met")	# 3100, 3098, 3100, 3100 and 96 vars
	...
	olink_panel$Assay=unlist(sapply(olink_panel$ab, function(x) subset(metadata.olink, id==x, Assay)))
	...
	olink.all <- Reduce(function(x,y) merge(x,y, by = "studyid", all = TRUE), list(olink[[1]][-c(2:4)],olink[[2]][-c(2:4)],olink[[3]][-c(2:4)],olink[[4]][-c(2:4)]))	# 3100  369	
...
for (a in 1:4){
	...
	for (i in 1:length(omics)){
		print(i)
		p=apply(omics[[i]][,6:ncol(omics[[i]])],2, function(x) summary(lm(omics[[i]][,1+a]~x))$coefficients[2,4] )
		q=p.adjust(p, method="fdr")
		beta=apply(omics[[i]][,6:ncol(omics[[i]])],2, function(x) summary(lm(omics[[i]][,1+a]~x))$coefficients[2,1] )
		df=data.frame(var=colnames(omics[[i]])[6:ncol(omics[[i]])], p=p, q=q, beta=beta)
		write.table(df, file=paste0(out.path,"Arch",a,".",names(omics)[i],".txt"),sep="\t", quote=F)
	}
}
