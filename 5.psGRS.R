...
for (a in 1:4){
  df=data.frame(archetype=rep(a,7), GRS=colnames(psGRS[6:12]), 
...
pdf("Fig3A.GRS_by_archetype.pdf",width=6,height=4)
...
#### test the psGRSs from Miriam 

grs=fread("~/FINAL_baseline_analysis_2019/WP2.2.psGRS_scores.txt")
grs=merge(archetypes, grs, by="studyid")

grs_by_archetype=data.frame()
for (a in 1:4){
  df=data.frame(archetype=rep(a,15), GRS=colnames(grs[9:23]), 
                p=sapply(1:15, function(x) summary(lm(grs[,1+a] ~ grs[,8+x]))$coefficients[8]),
                beta=sapply(1:15, function(x) summary(lm(grs[,1+a] ~ grs[,8+x]))$coefficients[2]) )
  grs_by_archetype=rbind(grs_by_archetype, df)
}

grs_by_archetype$logP=-log10(grs_by_archetype$p) * grs_by_archetype$beta/abs(grs_by_archetype$beta)
grs_by_archetype$star=cut(grs_by_archetype$p, breaks=c(-Inf, 0.001, 0.01, 0.05, Inf), label=c("***", "**", "*", "")) 
### similar results to Anubha's GRSs
